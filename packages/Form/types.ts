...
export declare type Form<T = Record<string, any>> = {
  ...
  form?: FormInstances<T> | FormInstance<T>;
...
export declare type FormItemDefault<T = Record<string, any>> = {
  name: keyof T;
...
export declare type FormItem<T = Record<string, any>> = {
...
export declare type FormInstance<T = Record<string, any>> = {
  getFieldError: (name: keyof T) => Promise<string | undefined>;
  getFieldsError: (
    names?: (keyof T)[],
  ...
  getFieldValue: (name: keyof T) => Promise<any>; //Get the value by the field name
  isFieldsTouched: (
    names?: (keyof T)[],
    allTouched?: boolean,
  ) => Promise<boolean>; //Check if fields have been operated. Check if all fields is touched when allTouched is true
  isFieldTouched: (name: keyof T) => Promise<boolean>; //Check if a field has been operated
  isFieldValidating: (name: keyof T) => Promise<boolean>; //Check field if is in validating
...
export declare type FormInstances<T = Record<string, any>> = {
  getFieldError: (
    name: keyof T,
  ) => Promise<(string | undefined)[] | string | undefined>;
  getFieldsError: (names?: (keyof T)[]) => Promise<ValueError[] | ValueError>; //Get the error messages by the fields name. Return as an array
  getFieldsValue: (
    names?: keyof T[],
    filter?: FilterGetValues,
  ...
  isFieldTouched: (name: keyof T) => Promise<boolean[] | boolean>; //Check if a field has been operated
  isFieldValidating: (name: keyof T) => Promise<boolean[] | boolean>; //Check field if is in validating
  resetFields: (fields?: (keyof T)[]) => Promise<void>; //Reset fields to initialValues
  setFields: (fields: FieldData<T>[]) => Promise<void>; //Set fields status
...
export declare type FieldData<K = string> = {
  error?: string; //Error messages
  name: K; //Field name path;
...
export declare type FormControl<T = Record<string, any>> = {
  setField: (v: ItemParams) => any;
  clearField: (name: keyof T, nameNew?: keyof T) => any;
  validateField: (v: ItemParams) => any;
  onChangeValue: (params: ParamsOnChangeValue) => any;
  setLayout: (config: {name: keyof T; layout: LayoutRectangle}) => any;
  renameLayout: (name: keyof T, name2: keyof T) => any;
  blurValidate: (name: keyof T) => any;
};
